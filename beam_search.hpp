...
constexpr int max_beam_depth = 170;
constexpr double time_limit = 2.7;
constexpr int max_beam_width = 800;
...
struct SearchTree {
    ...
    State current_state;
    Stack<Node, (int)1e7>* nodes;  // 追加したら動かさない・消さない  // 実体だとセグフォる…  // 確保に時間かかってる？
    ...
    inline SearchTree(const State& initial_state) :
        ...
        nodes(),
        ptr_current_node() {
        nodes = new Stack<Node, (int)1e7>(1);
    ...
    inline void ChangeState(Node* ptr_target_node) {
        ...
        int depth_diff = ptr_target_node->depth - ptr_current_node->depth;
